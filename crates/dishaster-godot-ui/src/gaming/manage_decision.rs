...
impl ManageDecisionGui {
    pub fn set_view(&mut self, view: &ManagementDecisionsView) {
        let provider = self
            .asset_provider
            .get()
            .expect("ManageDecisionGui asset provider not set");
...
impl DecisionOptionItem {
    pub fn set_view(&mut self, view: &ManagementDecisionView, provider: &AssetProvider) {
